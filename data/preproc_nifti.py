...
def main(args):
  ...
  nameparts = re.findall('(De_.+?)/', inputfolder) #should work most of the time...
  lenName = re.findall('fourCm|sixCm', inputfolder)
  number = re.findall('\d+_\d+', inputfolder) #should work most of the time...
  file_path= os.path.join(outputfolder, nameparts[0])
  if not os.path.exists(file_path):
        os.mkdir(file_path)
  file_path = os.path.join(file_path, lenName[0])
  if not os.path.exists(file_path):
        os.mkdir(file_path)
  out_uninterp = os.path.join(file_path, number[0] + "raw.nii.gz")
  outputpath = os.path.join(file_path, number[0] + ".nii.gz")
